...
let iconsContainer = document.getElementById('iconsContainer')
let changeIcon = document.querySelector('.changeIcon')
let searchIcon = document.getElementById('searchIcon')
searchIcon.onchange = searchIcon.onselectionchange = (e) => {
    console.log(e.target.value)
    let value = e.target.value
    let filteredIcons =  filterIcons(value)
    for (let icon of Object.keys(icons)) {
      iconsContainer.querySelector('.'+icon).parentNode.style.display = 'inline'
    }
    for (let filteredIcon of filteredIcons) {
      iconsContainer.querySelector('.'+filteredIcon).parentNode.style.display = 'none'
    }
}
changeIcon.addEventListener('click', (arg) => {
  iconsContainer.classList.toggle('active') 
})
for(let key of Object.keys(icons)){
  iconsContainer.insertAdjacentHTML('beforeend', `<button onclick="setIcon('${key}')"><i class='bi ${key}'></i></button>`)
}

function setIcon(key) {
  let ele = document.querySelector('.changeIcon i')
  let iconN = ele.classList[1]
  ele.classList.remove(iconN)
...
return Object.keys(icons).filter((item) => {
    return !item.includes(val.toLowerCase())
})
}
